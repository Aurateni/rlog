...
def index():
    ...
    return render_template('index.html', title='Главная страница', form=form, posts=posts.items, next_url=next_url,
                           prev_url=prev_url)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash('Поздравляю, вы зарегестрировались!')
        return redirect(url_for('login'))
...
def explore():
    ...
    prev_url = url_for('explore', page=posts.prev_num) \
        if posts.has_prev else None
    return render_template('index.html', title='Исследовать', posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def reset_password_request():
    ...
    if form.validate_on_submit():
        ...
        flash('Введите свой ямайл и мы пришлём инструкции')
        return redirect(url_for('login'))
...
def reset_password(token):
    ...
    if form.validate_on_submit():
        user.set_password(form.password.data)
        db.session.commit()
        flash('Ура, вы сделали это, вы сбросили пароль.')
        return redirect(url_for('login'))
